...
def multipliziere(worry, todo):
    operand = todo.split(" * ")[1]
    if operand == "old":
        return worry * worry
    else:
        return worry * int(operand)
...
def teile_und_runde(wert, faktor):
    result = int(wert / faktor)
    return result
...
def wohin(worry, todo, divisor, next_wahr, next_falsch):
    ...
    """
    calculate where this item goes next
    :param worry:
    :param todo:
    :return: monkey ID and new worry-level
    """
    if "+" in todo:
        new_worry = addiere(worry, todo)
    elif "*" in todo:
        new_worry = multipliziere(worry, todo)
    else:
        print("In wohin: Unknown operation: {}".format(todo))
        new_worry = 0

    new_worry = teile_und_runde(new_worry, 3)
